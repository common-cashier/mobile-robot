...
def go_to_receipt():
    if self(resourceId="com.chinamworld.main:id/text_two").exists(timeout=30):
...
def transfer():
    ...
    settings.log('press amount key: %s %s' % (settings.transferee.amount, datetime.datetime.now()), settings.Level.COMMON)
    self.sleep(3)
